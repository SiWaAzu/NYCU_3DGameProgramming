...
void BasicTutorial_00::setMassSpringSystem()
{
		...
		for (int i = 0; i < numSamples; ++i) 
		{
			...
			{
				int particleIndex = m_MassSpringSystem->addParticle(
					mSceneNode[j  * numSamples + i],
					mSmallSphereRadius,
					true
				);
				// std::cout <<  particleIndex << " : " << i * numCircles + j << std::endl;
			}
		}
		...
		for (int i = 0; i < numSamples; ++i) {
			for (int j = 0; j <  numCircles - 1; ++j) {
				int now = i * numCircles + j;
				//SPRING *spr1 = m_MassSpringSystem->addSpring(now, now  + 1);
				SPRING *spr1 = m_MassSpringSystem->addSpring(now + 1, now);
				
				if(j < numCircles - 2) {
					//SPRING *spr2 = m_MassSpringSystem->addSpring(now, now + 2);
					SPRING *spr2 = m_MassSpringSystem->addSpring(now + 2, now);
				}
			}
		}
}

void BasicTutorial_00::setObjPositions(int numSamples,int numCircles)
{
	...
	for (int j = 0; j < numCircles; ++j) {
        ...
        for (int i = 0; i < numSamples; ++i) {
			...
			float z = r * sin(a);
			float y = 100;
            mSceneNode[j * numSamples + i]->setPosition(x, y, z);
        }
    }
}
...
void BasicTutorial_00::setBackground()
{
	...
	stringstream ss;
	String msg;
	ss << m_MassSpringSystem;
	ss >> msg;
	msg += ":*** m_MassSpringSystem ***\n";
